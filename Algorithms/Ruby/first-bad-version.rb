...
#
# You are given an API bool is_bad_version?(version) which will return whether version is bad. Implement a function to
...
def first_bad_version(versions)
  ...
  is_bad_version?(middle_version) ? first_bad_version(versions[0..midpoint]) : midpoint + first_bad_version(versions[midpoint..-1])
end
